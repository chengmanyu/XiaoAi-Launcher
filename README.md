...
一個專為 Windows 設計的輕量工具，讓你可以用 **快速鍵** 或**語音喚醒**（說「小愛小愛」或「小愛」）快速呼叫「小愛同學電腦版」，並自動點擊語音輸入按鈕。
...
- **V3**: Vosk 離線版，可自訂熱鍵 + 可編輯喚醒詞，支援離線語音辨識（需下載 Vosk 模型）

## 功能比較

| 功能項目               | V1 版本                  | V2 版本                        |V3 版本（推薦）
|-----------------------|--------------------------|--------------------------------|-------------------------|
| 按住 F5 1秒 啟動       | ✓                        | ✓                             |✓（可自訂熱鍵，如 ctrl+1   |
| 語音喚醒（小愛/小愛同學/小愛小愛）| ✓               | ✓（更靈敏、反應更快）           |✓（離線辨識 + 可自訂喚醒詞）|
| 自動點擊語音按鈕       | ✓（固定比例座標）         | ✓（支援快取 + 手動校準）         |✓（支援快取 + 手動校準）     |
| 滑鼠鎖定防誤觸         | ✗                        | ✓（點擊期間約 1 秒鎖鼠）        |✓（點擊期間約 1.2 秒鎖鼠）  |
| 系統托盤圖示與選單     | ✓                        | ✓（可切換語音喚醒、重新校準）    |✓（新增設定熱鍵/喚醒詞選項）  |
| 位置校準功能           | ✗                        | ✓                             |✓                        |
| 命令列參數（--no-voice 等）| ✓                     | ✓                             |✓                        |
|離線語音辨識             |✗                         |✗                              |✓（使用 Vosk 模型）       |
